...
def calculate_demographic_data(print_data=True):
    ...
    average_age_men = df.groupby('sex').get_group('Male').age.mean().round(1)

    # What is the percentage of people who have a Bachelor's degree?
    percentage_bachelors =  round((df.groupby('education').get_group('Bachelors').shape[0]/len(df.index))*100, 1)
    ...
    rich_percentage = round( num_min_workers / df.groupby('hours-per-week').get_group(df['hours-per-week'].min()).shape[0]*100, 1)
